...
user_dict = {'name': 'Ashok', 'groups': ['wheel', 'staff'], 'password': password}
...
def test_users_add(mocker):
    ...
    users.add(user_dict)
    subprocess.call.assert_called_with(['useradd', '-p', password, '-G', 'wheel,staff', 'Ashok',])
...
def test_users_remove(mocker):
    ...
    subprocess.call.assert_called_with(['userdel', '-r', 'Ashok',])
...
def test_users_update(mocker):
    ...
    subprocess.call.assert_called_with(['usermod', '-p', password, '-G', 'wheel,staff','Ashok',])
...
def test_users_sync(mocker):
    ...
    subprocess.call.assert_has_calls([
        mocker.call(['usermod', '-p', password, '-G', 'wheel,staff', 'Ashok',]),
